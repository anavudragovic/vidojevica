...
def doAstrometry(fl):
        ...
        pixel = 0.39 * u.arcsec  # known pixel scale
        hdul = fits.open(fl)
        header = hdul[0].header
        data = hdul[0].data
        ...
        FITSout=FITSnm.replace(".fit","_wcs.fit" ).replace('calibrated','astrometry')
        print("Astrometry: ",FITSnm,' --> ',FITSout)
...
if __name__ == "__main__":
    ...
    print(len(inputFls_list), len(outputFls_list))
    if len(outputFls_list) < len(inputFls_list):
      #print("start astrometry:")
      tmp = list(map(lambda x: x.replace('_wcs','').replace('astrometry','calibrated'),outputFls_list))
      inputFls_list_new =  [x for x in inputFls_list if x not in set(tmp)]
      pool = mp.Pool(processes=mp.cpu_count()-1)
      for fl in inputFls_list_new:
          #print(fl)
          pool.apply_async(doAstrometry, args=(fl,))
          #mp.Process(target=doAstrometry, args=(lock, fl)).start()
      #doAstrometry(inputFls_list)
      pool.close()
      pool.join()
